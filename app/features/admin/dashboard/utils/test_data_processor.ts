...
export function process3In1TestData(test: TestData): TableRowData[] {
  ...
  // Process Visual test
  const visualTest = test.DETAILS?.find(
    (detail) => detail.TEST_NAME?.toLowerCase() === "visual_test"
  );

  if (visualTest?.DATAS) {
    for (const visualData of visualTest.DATAS) {
      const serialNo = visualData.SUB_COMPONENT_SERIAL_NO ?? "";
      const qcStatus = normalizeQcStatus(visualData.QC_STATUS);
  ...
  // Process 24V test (Valves)
  const valve24vTests =
    test.DETAILS?.filter(
      (detail) => detail.TEST_NAME?.toLowerCase() === "24v_test"
    ) ?? [];

  for (const valve24vTest of valve24vTests) {
    if (valve24vTest.DATAS) {
      for (const valveData of valve24vTest.DATAS) {
        const serialNo = valveData.SUB_COMPONENT_SERIAL_NO ?? "";
        const qcStatus = normalizeQcStatus(valveData.QC_STATUS);
        const inspection = valveData.INSPECTION ?? "N/A";

        tableData.push({
          srNo: serialNo || "N/A",
          componentName,
          testName: `24V Test (${inspection})`,
          dateTime: formatMillis(timestamp),
          qcStatus,
          downloadData: {
            type: "3-in-1",
  ...
  // Process 230V test (Heaters/Boilers)
  const heater230vTests =
    test.DETAILS?.filter(
      (detail) => detail.TEST_NAME?.toLowerCase() === "230v_test"
    ) ?? [];

  for (const heater230vTest of heater230vTests) {
    if (heater230vTest.DATAS) {
      for (const heaterData of heater230vTest.DATAS) {
        const serialNo = heaterData.SUB_COMPONENT_SERIAL_NO ?? "";
        const qcStatus = normalizeQcStatus(heaterData.QC_STATUS);
        const inspection = heaterData.INSPECTION ?? "N/A";

        tableData.push({
          srNo: serialNo || "N/A",
          componentName,
          testName: `230V Test (${inspection})`,
          dateTime: formatMillis(timestamp),
          qcStatus,
          downloadData: {
            type: "3-in-1",
  ...
  // Process Motors test (Pumps)
  const motorsTests =
    test.DETAILS?.filter(
      (detail) => detail.TEST_NAME?.toLowerCase() === "motors_test"
    ) ?? [];

  for (const motorsTest of motorsTests) {
    if (motorsTest.DATAS) {
      for (const motorData of motorsTest.DATAS) {
        const serialNo = motorData.SUB_COMPONENT_SERIAL_NO ?? "";
        const qcStatus = normalizeQcStatus(motorData.QC_STATUS);
        const inspection = motorData.INSPECTION ?? "N/A";

        tableData.push({
          srNo: serialNo || "N/A",
          componentName,
          testName: `Motors Test (${inspection})`,
          dateTime: formatMillis(timestamp),
          qcStatus,
          downloadData: {
            type: "3-in-1",
  ...
  // Process Sensors test
  const sensorsTests =
    test.DETAILS?.filter(
      (detail) => detail.TEST_NAME?.toLowerCase() === "sensors_test"
    ) ?? [];

  for (const sensorsTest of sensorsTests) {
    if (sensorsTest.DATAS) {
      for (const sensorData of sensorsTest.DATAS) {
        const serialNo = sensorData.SUB_COMPONENT_SERIAL_NO ?? "";
        const qcStatus = normalizeQcStatus(sensorData.QC_STATUS);
        const inspection = sensorData.INSPECTION ?? "N/A";

        tableData.push({
          srNo: serialNo || "N/A",
          componentName,
          testName: `Sensors Test (${inspection})`,
          dateTime: formatMillis(timestamp),
          qcStatus,
          downloadData: {
            type: "3-in-1",
  ...
  // Process Engineering Remarks
  const enggRemarksTest = test.DETAILS?.find(
    (detail) => detail.TEST_NAME?.toLowerCase() === "engg_remarks"
  );

  if (enggRemarksTest?.DATAS) {
    for (const remarksData of enggRemarksTest.DATAS) {
      const serialNo = remarksData.SUB_COMPONENT_SERIAL_NO ?? "";
      const qcStatus = normalizeQcStatus(remarksData.QC_STATUS);

      tableData.push({
        srNo: serialNo || "N/A",
        componentName,
        testName: "Engineering Remarks",
        dateTime: formatMillis(timestamp),
        qcStatus,
        downloadData: {
          type: "3-in-1",
